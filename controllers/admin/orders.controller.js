...
function hasPermission(res, permission) {
    return res.locals.roleUser && Array.isArray(res.locals.roleUser.permissions) && res.locals.roleUser.permissions.includes(permission);
}
        ...
        try {
            ...
            if (!order) {
                return respond(req, res, {
                    ...
                    json: {
                        success: false,
                        message: "Đơn hàng không tồn tại hoặc đã bị khách huỷ!"
                    },
                    redirect: "/admin/orders"
                });
            }
